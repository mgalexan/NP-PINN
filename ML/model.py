...
class ResidualBlock(nn.Module):
    def __init__(self, dim, act=nn.SiLU, norm=True, dropout=0.0):
        super().__init__()
...
class FourierFeatures(nn.Module):
    """ Random Fourier features embedding for coords (time, x, y) """
    def __init__(self, in_dim, mapping_size=64, scale=10.0):
        super().__init__()
        self.B = nn.Parameter(t.randn(in_dim, mapping_size) * scale, requires_grad=False)
        self.out_dim = mapping_size * 2

    def forward(self, x):
        # x: (N, in_dim)
        proj = 2 * t.pi * x @ self.B  # (N, mapping_size)
        return t.cat([t.sin(proj), t.cos(proj)], dim=-1)
...
class ForwardPINN(nn.Module):
    ...
    def __init__(self, env: ParamSpace, param_obj: MLParams):
        ...
        if param_obj["loss"] in {"Conc_Loss_Forward", "Conc_Loss_Backward", "Conc_Loss_CN"}:
            self.env.get_torch_funcs()
            self.get_coloc_points(param_obj["coloc_method"], param_obj["num_coloc"])
        
        if param_obj["loss"] in {"Conc_Loss_Backward", "Conc_Loss_CN"}:
...
class SplitModel(ForwardPINN):
    """A class that gives each output its own sub-network"""
    ...
    def make_layers(self) -> nn.ModuleList:
        ...
        for _ in range(self.out_size):
            ...
            out_layer = nn.Linear(self.size_hidden, 1)
    ...
    def forward(self, x: t.Tensor) -> t.Tensor:
        ...
        res = t.cat(outputs, dim=1)

        return res

class OnlyCNModel(ForwardPINN):
    """A class that only produces values of C_N"""
    ...
    def forward(self, x: t.Tensor) -> t.Tensor:
        ...
        res = self.network(y)
        out = t.zeros(len(res), 3, device=res.device, dtype=res.dtype)
        out[:, 0] = res[:, 0]


        return out

class ForwardPINNRadial(nn.Module):
    """ A PINN to handle the forwards case of learning the dynamics"""
    ...
    def __init__(self, env: ParamSpace, param_obj: MLParams):
        ...
        self.rscale = 1 / self.env.geometry.width
        self.tscale = 1 / self.env.geometry.T     
        if param_obj["loss"] == "Pressure_Loss_Radial":
            self.env.get_torch_funcs()
            self.get_coloc_points(param_obj["coloc_method"], param_obj["num_coloc"])
        if param_obj["loss"] in {"Conc_Loss_Backward_Radial", "Conc_Loss_Backward_Radial", "Conc_Loss_CN"}:
            self.env.get_torch_funcs()
            self.get_coloc_points(param_obj["coloc_method"], param_obj["num_coloc"])
        
        if param_obj["loss"] in {"Conc_Loss_Backward_Radial", "Conc_Loss_CN"}:
            self.alpha = nn.parameter.Parameter(t.ones(1))
            self.tau = nn.parameter.Parameter(t.ones(1))
            self.d = nn.parameter.Parameter(t.ones(1))
        self.to(self.device)
    ...
    def get_coloc_points(self, method= "grid", num_points = 1000):
        ...
        if method == "grid":
            if self.in_size == 1:
                rvals = t.linspace(0, self.env.geometry.width, num_points)
                coords = rvals.unsqueeze(-1).to(self.device)
                self.coloc = coords.requires_grad_(requires_grad=True)
            elif self.in_size == 2:
                n_side = int(np.sqrt(num_points))
                tvals = t.linspace(0, self.env.geometry.T, n_side)
                rvals = t.linspace(0, self.env.geometry.width, n_side)
                coords = t.stack(t.meshgrid(tvals, rvals, indexing="ij"), -1)
                coords = coords.reshape(-1, 2).to(self.device)
                self.coloc = coords.requires_grad_(requires_grad=True)
        elif method == "grid_time_sparse":
            if self.in_size == 1:
            ...
            for flag in self.env.flag_fun_lists["tumor"]:
                if type(flag) == SphericalFlag:
                    ...
                    break
            r = t.normal(mean=R, std=0.5, size=(n_spatial,))
            r = t.clamp(r, min=0.0, max=float(self.env.geometry.width))

            if self.in_size == 1:
                coords = r.unsqueeze(-1)
                self.coloc = coords.to(self.device).requires_grad_(True)
            elif self.in_size == 2:
                tvals = t.linspace(0, self.env.geometry.T, n_time)
                coords_repeat = r.unsqueeze(-1).repeat(n_time, 1)
                time_repeat = tvals.repeat_interleave(n_spatial)
                grid = t.cat((time_repeat[:, None], coords_repeat), dim=1)
                self.coloc = grid.to(self.device).requires_grad_(True)
                print(self.coloc.isnan().any())
            ...
            if self.in_size == 1:
                rvals = t.rand(num_points) * self.env.geometry.width
                coords = rvals.unsqueeze(-1).to(self.device)
                self.coloc = coords.requires_grad_(requires_grad=True)
            elif self.in_size == 2:
                tvals = t.rand(num_points) * self.env.geometry.T
                rvals = t.rand(num_points) * self.env.geometry.width
                coords = t.stack([tvals, rvals], -1).to(self.device).requires_grad_(True)
                self.coloc = coords
    def make_layers(self) -> nn.Sequential:
    ...
    def forward(self, x: t.Tensor) -> t.Tensor:
        ...
        if self.in_size == 2:
            ...
            raise ValueError(f"Unsupported in_size for ForwardPINNRadial: {self.in_size}")
        res = self.network(y)
